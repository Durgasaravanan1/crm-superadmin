...
//   Download, CheckCircle, AlertCircle, X, Info
// } from "lucide-react";
...
//           t.name, t.plan, `$${t.mrr}`, t.status === "suspended" ? "Failed" : "Current", "Jul 1, 2025"
//         ])
//       ];
...
//                   <span className={`text-xs font-mono px-2 py-0.5 rounded-lg border ${t.plan === "Scale" ? "text-violet-700 bg-violet-50 border-violet-200" : t.plan === "Grow" ? "text-cyan-700 bg-cyan-50 border-cyan-200" : "text-gray-600 bg-gray-50 border-gray-200"}`}>{t.plan}</span>
//                 </td>
//                 <td className="px-5 py-3.5 text-sm font-mono text-emerald-600 font-semibold">${t.mrr.toLocaleString()}</td>
//                 <td className="px-5 py-3.5 text-xs font-mono text-gray-400">Jul 1, 2025</td>
//                 <td className="px-5 py-3.5">
//                   <span className={`text-xs font-mono px-2 py-0.5 rounded-full border ${t.status === "suspended" ? "text-red-600 bg-red-50 border-red-200" : "text-emerald-600 bg-emerald-50 border-emerald-200"}`}>
//                     {t.status === "suspended" ? "Failed" : "Current"}
//                   </span>
//                 </td>
//               </tr>
//             ))}
...
const Billing = () => {
    ...
    try {
      const rows = [
        ...
        [
          ...
          tenant.plan, 
          `$${tenant.mrr}`, 
          tenant.status === "suspended" ? "Failed" : "Current", 
          "Jul 1, 2025",
  ...
  const sendEmailInvoice = (tenant) => {
    ...
    showPopup("info", "Sending Email", `Preparing to send invoice to ${tenant.email}...`);
    
    // Simulate email send delay
    setTimeout(() => {
      showPopup("success", "Email Sent!", `Invoice sent successfully to ${tenant.email}`);
    }, 1500);
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden overflow-x-auto">
              ...
              <tr key={t.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-5 py-3.5">
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${
                    ...
                    {t.status === "suspended" ? <AlertCircle className="w-3 h-3" /> : <CheckCircle className="w-3 h-3" />}
                    {t.status === "suspended" ? "Failed" : "Current"}
                  </span>
                </td>
                <td className="px-5 py-3.5">
        ...
        {tenants.filter(t => t.mrr > 0).length === 0 && (
